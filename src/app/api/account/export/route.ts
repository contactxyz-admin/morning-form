 ...
 *
 * Flow (plan Unit 5): auth → DB-backed rate-limit (2 non-failed/24h) → create
 * `pending` row → unconditional "export requested" notice email → assemble
 * archive → upload to private Blob → mark `complete` + send download-link email
 * pointing at the in-app download proxy (never the bare blob URL).
 *
 * Any assembly/upload failure marks the row `failed` with a reason, sends NO
 * download email, and returns an error — never a silent partial archive. A
 * `failed` request does not consume a rate-limit slot (it isn't counted).
 */
export async function POST() {
  ...
  const recent = await prisma.exportRequest.findMany({
    where: {
      userId: user.id,
      status: { not: 'failed' },
  ...
  try {
    ...
    const downloadUrl = `${env.NEXT_PUBLIC_APP_URL}/api/account/export/download?id=${request.id}`;
    await sendEmail({
      to: user.email,
