...
describe('POST /api/health/connect — provider access gating', () => {
  it('returns 401 before provider access details when unauthenticated', async () => {
  ...
  it('returns application-required for Garmin without creating a connection', async () => {
    const res = await POST(makeRequest({ provider: 'garmin' }));
    ...
    expect(body).toMatchObject({
      error: 'Garmin direct access is pending Garmin Connect Developer Program approval.',
  ...
  it('returns native-required for Apple Health without creating a connection', async () => {
    const res = await POST(makeRequest({ provider: 'apple_health' }));
    const body = await res.json() as {
      error: string;
      code: string;
      provider: string;
    ...
    expect(res.status).toBe(400);
    expect(body).toMatchObject({
      error: 'Apple Health requires the Morning Form iPhone app and cannot be connected from the web.',
      code: 'provider_native_required',
