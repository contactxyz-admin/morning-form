...
- **New `User.hasData Boolean @default(false)`** — O(1) memoization of the U17 new-user predicate. Flipped to `true` within the same transaction that commits the first row into any of: `HealthDataPoint`, `CheckIn`, `ProtocolItem`, `ProtocolAdjustment`, `AssessmentResponse`. Write-path pattern: `UPDATE User SET hasData = true WHERE id = ? AND hasData = false` — idempotent after the first flip, zero cost on subsequent writes. The five write paths (U6/U7 health points, U15 check-in projection, U16 protocol projection, existing assessment submit) must piggy-back this update inside their existing `prisma.$transaction` blocks. U17 reads this field instead of running five `COUNT(*)` queries per verify.
...
- `User.hasData` defaults to `false` on user creation; flips to `true` on the first insert into any of the five observed tables; stays `true` after subsequent inserts; never flips back.
...
- **Anthropic data-handling posture.** Client sets Anthropic zero-retention config (`metadata.user_id` for abuse tracking without training retention) on every request. Contractual protections — zero-retention tier, no training on customer data, cross-border transfer mechanism — live in the **executed DPA with Anthropic**, not in per-request headers that Anthropic may or may not honour as customer-controlled. **The DPA gate is artifact-backed, not a self-attested boolean.** Boot-time check: read `docs/legal/anthropic-dpa-signed.pdf` from the repo (committed after legal countersigns), compute `sha256`, assert it matches the `ANTHROPIC_DPA_SHA256` env value. Failure modes that refuse boot in production: file missing, hash mismatch, env unset. Keeps the gate auditable via git history — "when was the DPA activated, by whom" answered by the commit that adds the file. Rotating the DPA requires a code change (new file + new hash in env) rather than flipping a boolean, which is the desired friction. Documents the DPA requirement (R19 / sub-processor disclosure gate).
...
- **Env:** `ANTHROPIC_API_KEY` (required in prod), `SESSION_SECRET` (from U0b), `ANTHROPIC_DPA_SHA256` (required in prod — SHA-256 of the committed DPA artifact at `docs/legal/anthropic-dpa-signed.pdf`; boot asserts the file's hash matches), `EDGE_CONFIG` (required in prod; source of truth for kill-switch), `MOCK_LLM=true` (dev-only deterministic mock). `MOCK_LLM` ignored in production (refuses to boot if both set).
...
- Startup in `NODE_ENV=production` with `ANTHROPIC_DPA_SHA256` unset → boot refusal with explicit error
- Startup in `NODE_ENV=production` with `docs/legal/anthropic-dpa-signed.pdf` missing → boot refusal naming the missing artifact
- Startup in `NODE_ENV=production` with DPA artifact present but its sha256 ≠ `ANTHROPIC_DPA_SHA256` → boot refusal with both hashes logged
- Startup in `NODE_ENV=production` with DPA artifact present and hash match → proceeds normally
...
**Verification:** All tests green; `tsc --noEmit` clean; live-API smoke test under `scripts/llm-smoke.ts` (manual, documented in comment); prod boot refuses when `ANTHROPIC_DPA_SHA256`, `EDGE_CONFIG`, or `SESSION_SECRET` is unset, when the DPA artifact is missing, or when the computed hash mismatches.
...
- **Trigger moves into the verify handler (U0a) after session creation (U0b).** Flow:
  1. `isNewUser(userId)` predicate (new file): returns `!user.hasData` — a single `SELECT hasData FROM User WHERE id = ?` read. The `hasData` field is an O(1) memoization maintained by the five observing write-paths per U1; this replaces the earlier five-`COUNT(*)` implementation. Stays correct because `hasData` is transactionally flipped inside the same `prisma.$transaction` that commits the first observed row, and the predicate is only consulted *after* U0b session creation (any write that arrives concurrently with verify is serialised by the DB). Unit test seeds each of the five tables in isolation and asserts `isNewUser` returns `false` after each.
...
- **New-user predicate (memoized)**: user with `hasData = false` → `isNewUser` returns true, `GraphMigrationState` rows created with instant `completedAt`, no jobs enqueued, no `?migrating=1` suffix, no banner
- **Predicate memoization correctness**: for each of `HealthDataPoint`, `CheckIn`, `ProtocolItem`, `ProtocolAdjustment`, `AssessmentResponse`, seed exactly one row in isolation → `User.hasData` is `true` and `isNewUser` returns `false`. Rolling back the seeding transaction leaves `hasData` as `false` (same-transaction flip invariant).
...
- **Sub-processor disclosure (new surface at `/settings/privacy`).** Explicit, named list:
  - **Anthropic PBC** — LLM inference for extraction, topic-page generation, daily brief, GP prep. US-based. Data processed under an executed DPA with zero-retention and no-training commitments (Anthropic's enterprise zero-retention tier; see U2 artifact-backed boot gate: committed `docs/legal/anthropic-dpa-signed.pdf` + `ANTHROPIC_DPA_SHA256` hash assertion). Cross-border transfer under UK-US Data Bridge / SCCs.
...
2. **Historical-data predicate (U17 trigger gate).** Evaluated **before** migration enqueue: a user is "new" iff `User.hasData === false`. The field is memoized on the `User` row (U1) and flipped to `true` inside the same `prisma.$transaction` that commits the first row into any of `HealthDataPoint`, `CheckIn`, `ProtocolItem`, `ProtocolAdjustment`, `AssessmentResponse` — so the predicate is a single-field read (O(1)) at verify time rather than five `COUNT(*)` queries. For a new user, write `GraphMigrationState` rows with instant `completedAt` for every source-kind (no work to do) and skip enqueue. For an existing user with any historical rows and no `GraphMigrationState` entries, enqueue deterministic + LLM backfill jobs per U17.
...
**Consequence.** The new-user predicate keeps first-time sign-ups out of the migration banner UI entirely — they have nothing to backfill, so the banner would be misleading. The predicate is an O(1) read of `User.hasData` (U1), gated behind U0b session — no count queries on the hot path. Assessment-gating plan needs a small amendment before execution: its redirect logic must run **after** the U17 predicate, and the migration banner appends to its redirect rather than overriding it.
