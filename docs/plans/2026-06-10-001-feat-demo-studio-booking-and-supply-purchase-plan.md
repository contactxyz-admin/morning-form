...
- **Content module is scanned — because the review made it so**: the planning pass wrongly assumed `src/lib` was under `SCAN_ROOTS` (it covers `src/components` and `src/app`, NOT `src/lib`); moving the canned copy out of `page.tsx` would have silently un-scanned it. The code-review pass caught this; the fix adds a scoped `src/lib/demo` root (deliberately not all of `src/lib`, whose policy modules quote forbidden phrases by design) plus a wiring test pinning both the root and the module's presence in the scan set. The filename still deliberately avoids the substring "fixtures" (the walker skips those names).
...
### Resolved by Code Review (2026-06-10, 9-angle pass)
- **`src/lib/demo` added to `SCAN_ROOTS` + wiring test** — the plan's "scanned by construction" claim was false until this fix (top finding; see the corrected decision above).
- **`SequenceState` carries the whole picked `DemoSlot`**, not an id to re-resolve: the card and confirmation bubble render the same object by construction, eliminating the `?? slots[0]` fallback (crash on empty slots) and the card-blank/confirmation-wrong divergence on an unknown id.
- **Slots refresh on every chip selection** (state, not mount-frozen memo) so a tab left open past midnight never offers stale "upcoming" dates.
- **Slot buttons use `ui/Chip`**, restoring the focus-visible ring that `globals.css`'s global `outline: none` suppresses; the page's question chips (pre-existing gap) gained explicit focus-visible classes.
- **Price constants renamed `DEMO_STUDIO_VISIT_PRICE` / `DEMO_SUPPLY_PRICE`** so a grep of the marketing SOT can't mistake preview fiction for launched pricing.
- **`sequence.citations` now flows through to the answer bubble** (the `never[]` tripwire is operational, not just documentary) and `upcomingSlots` lost its unused `count` param (which silently recycled slot times past 3).
- Refuted during the same review, for the record: SSR hydration mismatch (slots never render pre-interaction), autoscroll-deps regression (`items.length` covers every reachable transition), dropped-`satisfies` weakening (return-type annotation is equivalent), DST date drift, SectionLabel/ui-Card swaps (demo idiom is intentional).
